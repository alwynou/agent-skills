...
Bootstrapping belongs to the entry points, not to Git hooks. Hooks live in `.git/hooks`, which is never cloned, so a hook cannot help the one case that needs help most: a clone on a new machine. Each entry point therefore repairs what it finds missing — dependencies and vendor submodules. Compiled output is not among them because none exists: every path runs the TypeScript sources through `tsx`, so the sources are the only artifact to keep correct.
